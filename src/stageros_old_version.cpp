...
private:
    ...
    struct StageRobot
    {
        ...
        ros::Publisher collision_pub; //one collision publisher
    ...
    ros::ServiceServer reset_srv_, teleport_srv_, unfreeze_srv_;
    ...
    bool apply_step_by_step = false, frozen = false;
...
StageNode::SubscribeModels()
{
	...
	// TODO - print the topic names nicely as well
        ...
        new_robot->collision_pub = n_.advertise<std_msgs::UInt8>(mapName(COLLISION, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10);
    ...
    if (false){
        unfreeze_srv_ = n_.advertiseService("step_stage", &StageNode::cb_unfreeze_srv, this);
    }
...
bool StageNode::teleport_service(stage_gym::teleport::Request& request, stage_gym::teleport::Response& response){
    ...
    for (size_t r = 0; r < this->positionmodels.size(); r++) {
        ...
        this->positionmodels[r]->SetVelocity(Stg::Velocity(0,0,0,0));
        this->positionmodels[r]->SetStall(false);
        // printf("R: %d->DONE\n", r);
    }
    // printf("After for\n");
  ...
  if( ! ros::ok() ) {
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        if(this->positionmodels[r]->Stalled()){
            ...
            msg_stall.data = 1;
            robotmodel->collision_pub.publish(msg_stall);
        }
